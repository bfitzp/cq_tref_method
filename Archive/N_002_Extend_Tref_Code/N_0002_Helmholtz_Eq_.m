function [u, maxerr, f, Z, Zplot, A] = N_0002_Helmholtz_Eq_(k_wave, P, z_0, varargin)
...
%     xlim([-0.1, 0.1]), ylim([-0.1, 0.1])
    ...
    for k = 1:nw
        Kk = find(Kj==k);
        errk(k) = norm(wt(Kk).*(A(Kk,:)*c-Gn(Kk)),2); % error near corner k
    end
    
    %----------------------------------------------------------------------
    % Global error
    %----------------------------------------------------------------------
    err = norm(wt.*(A*c-Gn),2);                     % global error
    rel_err = err/norm(Gn,inf);
    fprintf('%d: rel_err: %.16f\n',stepno,rel_err);
    %     polmax = 100;
    polmax = 100;
